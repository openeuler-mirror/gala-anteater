...
class RootCauseInfo(BaseModel):
    """根因信息：描述单条异常的可能根因"""
    metric: str = Field(default="", description="异常指标名称（例如：gala_gopher_sli_container_cpu_busy）")
    labels: Dict[str, Union[str, int, float]] = Field(default_factory=dict, description="根因指标的详细标签，如container_name、machine_id等")
...
class AnomalyInfo(BaseModel):
    ...
    labels: Dict[str, Union[str, int, float]] = Field(default_factory=dict, description="异常指标的标签信息（container_id、instance等）")
    score: float = Field(default=0.0, description="异常分数（越高说明异常程度越高）")
    entity_name: Optional[str] = Field(default="", description="实体名（如指标的entity_name）")
    details: Dict[str, Union[str, int, float, dict]] = Field(default_factory=dict, description="检测附加信息（例如容器额外信息）")


class PerceptionResult(BaseModel):
    """容器干扰检测结果"""
    is_anomaly: bool = Field(default=False, description="是否存在异常")
    anomaly_info: List[AnomalyInfo] = Field(default_factory=list, description="异常节点详细列表")
    start_time: int = Field(default=0, description="检测窗口开始时间戳（秒）")
    end_time: int = Field(default=0, description="检测窗口结束时间戳（秒）")
