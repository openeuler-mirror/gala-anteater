...
class TSDBSCAN:
    def __init__(self, config):
        self.eps = config.get("eps", 0.03)
        self.min_samples = config.get("min_samples", 5)
        self.detect_type = config.get("detect_type", "bi_bound")

    def detect(self, ts_data):
        ts_data = min_max_processing(ts_data)
