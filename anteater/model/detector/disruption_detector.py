...
class ContainerDisruptionDetector(Detector):
    def __init__(self, data_loader: MetricLoader, config: ModelConfig, **kwargs):
        ...
        logger.info(f"配置参数: q={self.q}, level={self.level}, smooth_win={self.smooth_win}")
    ...
    def _execute(self, kpis: List[KPI], features: List[Feature], **kwargs) \
            -> List[Anomaly]:
        logger.info("=" * 80)
        logger.info("【模块1-入口】开始执行ContainerDisruptionDetector._execute()")
        logger.info(f"接收到的KPI数量: {len(kpis)}")
        logger.info(f"接收到的Feature数量: {len(features)}")
        logger.info(f"额外参数kwargs: {kwargs}")
    ...
    def detect_and_rca(self, kpis: List[KPI]):
        ...
        for idx, k in enumerate(kpis):
            logger.info(f"  KPI[{idx}]: metric={k.metric}, entity_name={k.entity_name}, params={k.params}")
        
        # 获取机器ID列表
        logger.info("【模块2.1-获取机器列表】开始获取唯一机器ID...")
        machine_ids = self.get_unique_machine_id(start, end, kpis)
        logger.info(f"【模块2.1-获取机器列表】成功获取机器ID列表: {machine_ids}")
        logger.info(f"机器总数: {len(machine_ids)}")
        
        # 遍历每台机器和每个KPI进行检测
        logger.info("【模块2.2-遍历检测】开始对每台机器的每个KPI进行检测...")
        anomalies = []
    ...
    def detect_signal_kpi(self, kpi, machine_id: str) -> List[Anomaly]:
        ...
        logger.info(f"【模块3.1-SPOT检测】SPOT检测完成，发现 {len(anomalies_spot)} 个异常")
        
        if anomalies_spot:
            anomalies.extend(anomalies_spot)
            logger.info(f"【模块3-单KPI检测】累计异常数: {len(anomalies)}")

        logger.info(f"【模块3-单KPI检测】detect_signal_kpi()完成，返回 {len(anomalies)} 个异常")
        return anomalies

    def get_kpi_ts_list(self, metric, machine_id: str, look_back):
        logger.info(f"【模块4-数据获取】进入get_kpi_ts_list()")
        logger.info(f"  参数: metric={metric}, machine_id={machine_id}, look_back={look_back}分钟")
        ...
        logger.info(f"【模块4-数据获取】获取到 {len(ts_list)} 条时间序列，期望数据点数: {point_count}")
    ...
    def detect_by_spot(self, kpi, machine_id: str) -> List[Anomaly]:
        ...
        if not ts_scores:
            logger.warning(f"【模块5-SPOT检测】关键指标 {kpi.metric} 在机器 {machine_id} 上为空!")
            return []

        logger.info(f"【模块5.2-过滤结果】原始结果数: {len(ts_scores)}")
        ts_scores = [t for t in ts_scores if t[1] >= outlier_ratio_th]
        logger.info(f"【模块5.2-过滤结果】过滤后结果数(score >= {outlier_ratio_th}): {len(ts_scores)}")
        ...
        logger.info(f"【模块5-SPOT检测】detect_by_spot()完成，返回 {len(anomalies)} 个异常")
        return anomalies
    ...
    def cal_spot_score(self, metric, machine_id: str, **kwargs) \
        ...
        logger.info("\n" + "-" * 80)
        logger.info(f"【模块6-SPOT分数计算】进入cal_spot_score()")
        
        look_back = kwargs.get('look_back')
        obs_size = kwargs.get('obs_size')
        logger.info(f"  参数: metric={metric}, machine_id={machine_id}")
        logger.info(f"  参数: look_back={look_back}分钟, obs_size={obs_size}")
        logger.info(f"  其他参数: {kwargs}")
        
        logger.info("【模块6.1-初始化TSDBSCAN】创建TSDBSCAN检测器...")
        ...
        logger.info("【模块6.4-遍历时间序列】开始逐个处理时间序列...")
        for ts_idx, _ts in enumerate(ts_list):
            logger.info(f"\n  >>> 处理时间序列 [{ts_idx + 1}/{len(ts_list)}] <<<")
            logger.info(f"  时间序列标签: {_ts.labels}")
            logger.info(f"  时间序列长度: {len(_ts.values)}")
            logger.info(f"  时间序列统计: sum={sum(_ts.values):.2f}, max={np.max(_ts.values):.2f}, "
                       f"min={np.min(_ts.values):.2f}, mean={np.mean(_ts.values):.2f}")
            
            # DBSCAN检测
            logger.info("  【步骤6.4.1-DBSCAN异常检测】运行TSDBSCAN检测...")
            detect_result = ts_dbscan_detector.detect(_ts.values)
            logger.info(f"  DBSCAN检测结果长度: {len(detect_result)}")
            logger.info(f"  正常点数: {sum(1 for x in detect_result if x == 0)}, "
            ...
            logger.info(f"  去重后训练数据: {len(train_dedup_values)}, 去重后测试数据: {len(dedup_values)}")
            logger.info(f"  训练数据重复率: {(1 - len(train_dedup_values)/len(train_data))*100:.2f}% "
            ...
            if len(_ts.values) < point_count * 0.6:
                skip_reasons.append(f"数据点过少({len(_ts.values)} < {point_count * 0.6})")
            ...
            if skip_reasons:
                ...
                logger.info("  【步骤6.4.5-SPOT算法执行】时间序列通过质量检查，开始SPOT检测...")
                ...
                for bound_idx, (_ts_series, _ts_series_train) in enumerate(zip(ts_series_list, ts_series_train_list)):
                ...
                logger.info(f"  【步骤6.4.6-结果统计】SPOT总检测结果: {result.tolist()}")
                ...
                if output >= 3:
                    ...
                    logger.info(f"    容器信息: container_name={container_hostname}, machine_id={machine_id}")
                    
                    logger.info("  【步骤6.4.7-获取额外信息】调用get_container_extra_info()...")
        ...
        logger.info(f"  异常数量: {sum(1 for _, score, _, _ in ts_scores if score > 0)}")
        logger.info("-" * 80 + "\n")
        return ts_scores
    ...
    def find_discruption_source(self, victim_ts: TimeSeries, all_ts: List[TimeSeries]) \
        ...
        if root_causes[:3]:
            for rc_idx, rc in enumerate(root_causes[:3]):
                logger.info(f"    根因[{rc_idx + 1}]: {rc.metric}, score={rc.score}, cpu_num={rc.labels.get('cpu_num')}")
        ...
        for metric_idx, metric in enumerate(extra_metrics):
            ...
            for _ts in ts_list:
                if container_name == _ts.labels.get('container_name', ''):
                    ...
                    logger.info(f"      appkey={result['appkey']}, cpu_num={result['cpu_num']}")
                    break
    ...
    def _check_bound_type(bound_type, metric_data):
        logger.info(f"    【辅助函数-边界类型】_check_bound_type(), bound_type={bound_type}")
    ...
    def cal_trend(metric_values: list, obs_size: int) -> float:
        ...
        logger.info(f"      趋势值: {result} ({'+' if result > 0 else ''}{result*100:.1f}%)")
